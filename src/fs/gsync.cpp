...
struct gsync {
	...
	void scan_and_compare()
	{
		diff_reset();

		if (!(this->lsnap = this->sync_if->scan(xxvec(this->wmain.lpath.text()).str(), 0))) {
			this->error("ERROR scanning Source tree");
			return;
		}

		if (!(this->rsnap = this->sync_if->scan(xxvec(this->wmain.rpath.text()).str(), 0))) {
			this->error("ERROR scanning Target tree");
			return;
		}
		...
		this->diff = this->sync_if->diff(this->lsnap, this->rsnap, &this->props, f);
	...
	void sync_begin()
	{
		...
		this->sync_prepare();
		if (this->sync_items.len)
			core->task(&this->core_task, sync__worker, this);
	}

	void sync_prepare()
	{
		xxvec indexes = this->wmain.view.selected();
		uint *it;
	...
	static void sync__worker(void *param)
	{
		...
		for (;  g->sync_item_i < g->sync_items.len;  g->sync_item_i++) {
			void *id = g->sync_items.at<void*>(g->sync_item_i);
			uint f = g->show_flags & FCOM_SYNC_SWAP;
