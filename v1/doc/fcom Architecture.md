...
fcom's architecture is modular.  Each module knows its own job and relies on other modules to perform their job.  Modules are linked to each other via Core.  The key idea here is to provide convenient interfaces for operation module to do its job, such as asynchronous I/O events delivery and fast timers.  The Core interacts with each module and provides the standard way for each module to find each other, talk to each other and send/receive asynchronous events to each other.  The languages such as Go already provide all this stuff by default via its Go-runtime.  C apps don't have such runtime layer by default - it's implemented in fcom as Core.

Here's an example how fcom initializes its core resources, how Core finds an operation handling function and how then this operation module finds a secondary operation interface from another module.  Although it may look complex, this is actually pretty fast, simple and straightforward code with very little overhead (a couple milliseconds at most) comparing to monolithic tiny executable files.
